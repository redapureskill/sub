	...
	def get_account_lines(self, data):
		...
		for report in child_reports: 
			if not income_balance:
				income_balance = res[report.id]['balance'] * float(report.sign) if report.name == 'Income' else 0
			percentage = (res[report.id]['balance'] * float(report.sign))/income_balance if income_balance else 0
			vals = {
				'name': report.name,
			...
			if res[report.id].get('account'):
				...
				for account_id, value in res[report.id]['account'].items():
					...
					percentage = (value['balance'] * float(report.sign) or 0.0)/income_balance if income_balance else 0
					vals = {
